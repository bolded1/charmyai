...
import { HardDrive, Plus, Minus, ShoppingCart, Loader2, CheckCircle2 } from "lucide-react";
...
export default function StorageSettings() {
  ...
  const progressColor = usedPercent >= 90 ? "bg-destructive" : usedPercent >= 70 ? "bg-yellow-500" : "bg-primary";
        ...
        <CardContent className="p-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between text-sm">
              ...
              <Badge variant={storage.isFull ? "destructive" : "secondary"} className="text-xs">
            ...
            {storage.isFull && (
              <div className="flex items-center gap-2 text-sm text-destructive bg-destructive/10 p-3 rounded-lg">
                ...
                <span className="text-destructive/70">You won't be able to upload new documents until you purchase more storage.</span>
              </div>
            )}

            <div className="grid grid-cols-3 gap-4 pt-2">
              <div className="text-center p-3 bg-muted/50 rounded-lg">
                <p className="text-lg font-bold text-foreground">{formatBytes(storage.limitBytes)}</p>
                <p className="text-[11px] text-muted-foreground">Base Storage</p>
              </div>
              <div className="text-center p-3 bg-muted/50 rounded-lg">
                <p className="text-lg font-bold text-foreground">{formatBytes(storage.purchasedBytes)}</p>
                <p className="text-[11px] text-muted-foreground">Extra Purchased</p>
              </div>
              <div className="text-center p-3 bg-muted/50 rounded-lg">
                <p className="text-lg font-bold text-foreground">{formatBytes(storage.remainingBytes)}</p>
      ...
      <Card>
        <CardContent className="p-6">
          <div className="flex items-center gap-2 mb-5">
            ...
            <h3 className="text-sm font-medium text-foreground">Purchase Extra Storage</h3>
          </div>

          <p className="text-sm text-muted-foreground mb-4">
            Add more storage to your workspace. Each 1 GB costs <strong>$10</strong> (one-time, lifetime).
          </p>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">One-time payment, permanent addition</p>
            </div>
            <div className="flex items-center gap-3">
              <span className="text-2xl font-bold text-foreground">${quantity * 10}</span>
              <Button onClick={handlePurchase} disabled={purchasing} className="rounded-xl">
                {purchasing ? (
