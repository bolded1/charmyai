...
import { FileText, Mail, Eye, EyeOff, ArrowRight, Loader2 } from "lucide-react";
...
export default function LoginPage() {
  ...
  return (
    <div className="marketing min-h-screen flex items-center justify-center p-4 relative overflow-hidden bg-background">
      {/* Background orbs */}
      <div className="absolute top-[-20%] left-[-10%] w-[500px] h-[500px] rounded-full opacity-20 blur-[100px]" style={{ background: 'radial-gradient(circle, hsl(224 76% 48% / 0.6), transparent 70%)' }} />
      <div className="absolute bottom-[-15%] right-[-5%] w-[400px] h-[400px] rounded-full opacity-15 blur-[100px]" style={{ background: 'radial-gradient(circle, hsl(262 83% 58% / 0.5), transparent 70%)' }} />
      ...
      >
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-[11px] text-primary/80 cursor-pointer hover:text-primary transition-colors font-medium"
                >
        ...
        <p className="text-center text-[13px] text-muted-foreground mt-6">
          ...
          <Link to="/signup" className="text-primary font-semibold hover:text-primary/80 transition-colors">{t("auth.getStarted")}</Link>
        </p>
      </motion.div>
      ...
      <Dialog open={forgotOpen} onOpenChange={setForgotOpen}>
        <DialogContent className="sm:max-w-[380px] rounded-2xl">
          <DialogHeader>
            <DialogTitle className="text-lg">{t("auth.resetPassword")}</DialogTitle>
          </DialogHeader>
          {forgotSent ? (
            <motion.div
            ...
            <form onSubmit={async (e) => {
              ...
              <div className="space-y-1.5">
                <Label htmlFor="forgot-email" className="text-xs font-medium text-muted-foreground">{t("auth.email")}</Label>
                <Input id="forgot-email" type="email" placeholder="you@company.com" required value={forgotEmail} onChange={(e) => setForgotEmail(e.target.value)} className="h-11 rounded-xl" />
              </div>
              <Button type="submit" className="w-full h-11 rounded-xl bg-hero-gradient hover:opacity-90 transition-opacity font-medium" disabled={forgotLoading}>
