...
export default function IncomePage() {
  ...
  const income = mockDocuments.filter((d) => d.type === 'sales_invoice');
  ...
  const total = filtered.reduce((sum, d) => sum + d.totalAmount, 0);
              ...
              <tbody>
                {filtered.map((doc) => (
                  <tr key={doc.id} className="border-b last:border-0 hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-3 text-sm text-muted-foreground">{doc.invoiceNumber}</td>
                    <td className="p-3 text-sm text-muted-foreground">{doc.date}</td>
                    <td className="p-3 text-sm text-muted-foreground">{doc.dueDate || '—'}</td>
                    <td className="p-3 text-sm">{doc.currency} {doc.netAmount.toFixed(2)}</td>
                    <td className="p-3 text-sm text-muted-foreground">{doc.currency} {doc.vatAmount.toFixed(2)}</td>
                    <td className="p-3 text-sm font-medium">{doc.currency} {doc.totalAmount.toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
